...
1.0.0 候选版面向 macOS，本地开箱即用，不需要登录：

- 支持一个项目关联多个本地目录，目录可以是代码、文档或空目录；没有 Git 的目录会在首个相关任务计划确认后初始化本地仓库。
- 内置产品、研发、测试、评审 Role；历史 Skill 库继续用于兼容 0.1 任务，但不再决定新任务流程。
...
- 能力中心只读发现 OpenCode、Claude Code 和通用 Agent 目录中的标准 Skill/MCP，分别展示来源、兼容 CLI、解析状态、命令存在性与运行健康，不把“能解析”误报成“可运行”。
