...
describe('v2 WorkUnit flow', () => {
  it('uses the team as a candidate pool and persists work-unit execution boundaries', () => {
    ...
    expect(task).toMatchObject({ flowVersion: 2, status: 'WAITING_PLAN_APPROVAL' });
    expect(task.steps).toEqual([
      expect.objectContaining({
        kind: 'work_unit', skillId: 'work-unit', agentId: productAgent.id, mode: 'write',
        requiredCapabilities: ['项目检索', '代码实现', '验证'],
        verification: ['检查 Git diff', '执行项目测试'],
      }),
    ]);
    ...
    expect(task.steps[0]).toMatchObject({ status: 'pending', attempt: 1, kind: 'work_unit', mode: 'write' });
    expect(store.getResumableExternalSession(task.id, productAgent.id)).toBe('opencode-session-1');
    database.close();
  });

  it('keeps an explicitly imported legacy Skill plan on flow version 1', () => {
    const root = mkdtempSync(join(tmpdir(), 'yanxu-legacy-plan-'));
    roots.push(root);
    const projectDirectory = join(root, 'project');
    mkdirSync(projectDirectory);
    const database = openDatabase(join(root, 'workbench', 'system', 'app.db'));
    ...
    const developer = store.createAgent({
      name: '旧版研发', roleId: 'development', executor: 'opencode', model: 'test/model',
    }, openCode);
    const team = store.createTeam({ name: '旧版团队', memberIds: [developer.id] });
    const project = store.createProject({ name: '旧版项目', directoryPath: projectDirectory });
    let task = store.createTask({
      projectId: project.id, teamId: team.id, title: '导入旧计划', description: '验证历史计划兼容。',
    });
    task = store.submitTask(task.id, task.stateVersion);
    task = store.saveComposedPlan(task.id, {
      questions: [],
      steps: [{
        id: 'legacy', position: 0, skillId: 'implementation', agentId: developer.id,
        title: '内容实施', description: '旧版 SkillStep', inputs: [], expectedOutput: 'ImplementationReport',
        directoryIds: [project.directories[0]!.id],
      }],
    });
    expect(task.flowVersion).toBe(1);
    expect(task.steps[0]).toMatchObject({ kind: 'legacy_skill', skillId: 'implementation' });
    database.close();
  });
  ...
  it('freezes different CLI runtimes and independent reviewer responsibility in one team plan', () => {
    ...
    task = store.saveComposedPlan(task.id, {
      ...
      steps: [
        {
          id: 'implementation', position: 0, skillId: 'work-unit', kind: 'work_unit', agentId: developer.id,
        ...
        {
          id: 'review', position: 1, skillId: 'work-unit', kind: 'work_unit', agentId: reviewer.id,
