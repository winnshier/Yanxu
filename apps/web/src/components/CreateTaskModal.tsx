  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" requiredMark="optional">
        ...
        <Form.Item name="title" label="任务标题" rules={[{ required: true, message: '请输入任务标题' }]}><Input placeholder="一句话说明要完成什么" /></Form.Item>
        <Form.Item name="description" label="需求描述" rules={[{ required: true, message: '请描述需求' }]}><Input.TextArea rows={6} placeholder="描述背景、功能和你关心的结果。计划阶段会继续澄清关键歧义。" /></Form.Item>
        <Form.Item name="expectedOutput" label="预期产出"><Input.TextArea rows={2} placeholder="选填，例如：可运行功能、接口文档和测试报告" /></Form.Item>
        <Form.Item name="constraints" label="约束"><Input.TextArea rows={2} placeholder="选填，例如：保持现有 API 兼容" /></Form.Item>
        <Form.Item name="forbiddenPathsText" label="禁止路径"><Input.TextArea rows={2} placeholder="选填，每行一个路径" /></Form.Item>
